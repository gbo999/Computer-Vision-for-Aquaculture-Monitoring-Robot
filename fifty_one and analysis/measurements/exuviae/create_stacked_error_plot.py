...
def create_stacked_error_plot():
    ...
    individual_data = merged_df[['image_name', 'category', 'signed_pixel_diff_pct', 'signed_measurement_diff_pct', 'individual_mae', 'individual_mape']].copy()
    individual_data = individual_data.round(3)
    print(individual_data.head(10))
    
    # Save individual prawn data
    individual_data.to_csv("individual_prawn_signed_errors.csv", index=False)
    print("Saved individual_prawn_signed_errors.csv")
    ...
    # Update layout
    fig.update_layout(
        title='Stacked Error Analysis: Signed Pixel vs Measurement Errors by Category',
    ...
    fig.write_html("stacked_error_plot.html")
    print("Saved stacked_error_plot.html")
    ...
    fig_individual.update_layout(
        title='Error Comparison: Signed Pixel vs Measurement Errors by Category',
        xaxis_title='Pond Type and Size',
        yaxis_title='Signed Error (%)',
        barmode='group',
        height=600,
    ...
    fig_individual.write_html("error_comparison_plot.html")
    print("Saved error_comparison_plot.html")
    ...
    fig_metrics.update_layout(
        title='MAE and MAPE by Category',
        xaxis_title='Pond Type and Size',
    ...
    fig_metrics.write_html("mae_mape_plot.html")
    print("Saved mae_mape_plot.html")
    ...
    comprehensive_df.to_csv("comprehensive_error_analysis.csv", index=False)
    print("\nSaved comprehensive_error_analysis.csv")
    ...
    fig_comprehensive.update_layout(
        title="Comprehensive Error Analysis with MAE and MAPE",
        width=1200,
        height=300
    )
    
    fig_comprehensive.write_html("comprehensive_error_analysis_table.html")
    print("Saved comprehensive_error_analysis_table.html")
...
def plot_error_components_by_pond_type(merged_df):
    ...
    fig.write_html('error_components_by_pond_type.html')
    print('Saved error_components_by_pond_type.html')
    ...
    fig.write_html('error_components_per_measurement.html')
    print('Saved error_components_per_measurement.html')
...
if __name__ == "__main__":
    ...
    try:
        ...
        import plotly.subplots as sp
        measurement_df = pd.read_csv("complete_detailed_results_table.csv")
        signed_df = pd.read_csv("individual_prawn_signed_errors.csv")
        merged_df = pd.merge(measurement_df, signed_df, on=["image_name"], how="inner")
        merged_df['category'] = merged_df['pond_type'] + '_' + merged_df['manual_size']
        merged_df = merged_df.reset_index(drop=True)
        merged_df['measurement_index'] = merged_df.index
        pond_groups = merged_df.groupby('category')
        ...
        fig.update_layout(
            title_text='Stacked Signed Error Components by Measurement (Percentage, Compounding/Cancellation)',
            barmode='relative',
            width=350*n_groups,
            height=400,
            legend=dict(x=1.05, y=1)
        )
        print('Writing error_components_stacked_signed_pct_per_measurement.html...')
        fig.write_html('error_components_stacked_signed_pct_per_measurement.html')
        print('Saved error_components_stacked_signed_pct_per_measurement.html')
    except Exception as e:
        print('Failed to create error_components_stacked_signed_pct_per_measurement.html:', e)
    ...
    try:
        ...
        for i, (cat, group) in enumerate(pond_groups, 1):
            group = group.sort_values('measurement_index')
            customdata = np.stack((group['pixel_error_pct'], group['scale_error_pct'], group['mape']), axis=-1)
            fig.add_trace(
                go.Bar(
                    x=group['measurement_index'],
                    ...
                    marker_color='royalblue',
                    customdata=customdata,
                    hovertemplate='Index: %{x}<br>Pixel Error: %{customdata[0]:.2f}%<br>Scale Error: %{customdata[1]:.2f}%<br>MAPE: %{customdata[2]:.2f}%',
                    showlegend=(i==1)
                ),
                row=1, col=i
            )
            ...
            fig.update_yaxes(title_text='Signed Error (%)', row=1, col=i)
        fig.update_layout(
            title_text='Stacked Pixel + Scale Error Components by Measurement (Percentage, Compounding/Cancellation)',
            barmode='relative',
            width=350*n_groups,
            height=400,
            legend=dict(x=1.05, y=1)
        )
        print('Writing error_components_stacked_pixel_scale_pct_per_measurement.html...')
        fig.write_html('error_components_stacked_pixel_scale_pct_per_measurement.html')
        print('Saved error_components_stacked_pixel_scale_pct_per_measurement.html')
    except Exception as e:
        print('Failed to create error_components_stacked_pixel_scale_pct_per_measurement.html:', e)
    print('Script completed.') 