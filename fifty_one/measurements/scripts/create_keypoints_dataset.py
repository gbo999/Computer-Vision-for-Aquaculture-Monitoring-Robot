...
LABELS_DIR = os.path.join(BASE_DIR, "runs/pose/predict81/labels")
...
CSV_FILE_SHAI = os.path.join(BASE_DIR, "fifty_one/measurements/data/Results-shai-exuviae.csv")
...
try:
    ...
    print("Dataset has been saved. You can also access it later with:")
    print("session = fo.launch_app(fo.load_dataset('prawn_keypoints'))")
except Exception as e:
    print(f"Error launching app: {e}")
    print("Dataset has been saved. You can still access it later with:")
    print("session = fo.launch_app(fo.load_dataset('prawn_keypoints'))")
if __name__ == "__main__":
    session = fo.launch_app(fo.load_dataset('prawn_keypoints'))
    session.wait()