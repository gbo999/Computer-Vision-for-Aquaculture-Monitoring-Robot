...
def calculate_prawn_measurements(points, manual_size, image_name):
    ...
    # Determine pond type and set uniform height per pond
    is_circle = "GX010191" in image_name
    
    # MODIFIED: Using uniform heights per pond
    if is_circle:  # Circle pond
        height_mm = 690  # Uniform height for Circle pond
    else:  # Square pond
        height_mm = 400  # Uniform height for Square pond
...
def find_matching_keypoints(sample, bbox_normalized):
    """Find keypoints that match the given bounding box"""
...
print("MODIFIED VERSION: Using uniform heights - Circle: 690mm, Square: 400mm")
...
for _, manual_row in manual_classified.iterrows():
    ...
    if fo_sample is not None:
        ...
        if keypoints_match:
            ...
            # Calculate measurements using uniform heights
...
output_filename = "merged_manual_shai_keypoints_uniform_heights.csv"
...
print(f"Circle Pond entries (690mm): {len(merged_df[merged_df['pond_type'] == 'Circle'])}")
print(f"Square Pond entries (400mm): {len(merged_df[merged_df['pond_type'] == 'Square'])}")
...
if len(circle_small) > 0:
    ...
    print(f"\nCircle Pond Small Exuviae (690mm height):")
...
if len(square_small) > 0:
    ...
    print(f"\nSquare Pond Small Exuviae (400mm height):")
