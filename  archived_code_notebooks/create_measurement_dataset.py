...
def create_measurement_dataset():
    ...
    merged_df = pd.read_csv("/Users/gilbenor/Documents/code_projects/msc/counting_research_algorithms/fifty_one and analysis/measurements/exuviae/spreadsheet_files/merged_manual_shai_keypoints.csv")
    ...
    for _, row in merged_df.iterrows():
        ...
        sample['image_name'] = image_name.split('_')[-1]
        ...
        for _, measurement in image_measurements.iterrows():
            ...
            if total_length_pixels is not None:
                error_difference = abs(shai_length_pixels - total_length_pixels)
                if size == 'big':
                    if error_difference > 150:  # Higher threshold for big exuviae
                        sample['tags'].append("Big_exuviae_big_error")
                    elif error_difference > 75:
                        sample['tags'].append("Big_exuviae_medium_error")
                    else:
                        sample['tags'].append("Big_exuviae_small_error")
                else:  # small
                    if error_difference > 100:  # Lower threshold for small exuviae
                        sample['tags'].append("Small_exuviae_big_error")
                    elif error_difference > 50:
                        sample['tags'].append("Small_exuviae_medium_error")
                    else:
                        sample['tags'].append("Small_exuviae_small_error")
            else:
                sample['tags'].append("No keypoints available")
        ...
        if detections:
            ...
            sample["tags"].append(pond_type.lower())
