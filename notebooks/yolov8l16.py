...
def draw_bounding_boxes(
    image: np.ndarray,image_file_name, ground_truth, predictions, class_names
) -> np.ndarray:
    """
...
def log_to_wandb(images_folder, ground_truth_folder, predictions_folder, class_names):
    ...
    for image_file in os.listdir(images_folder):
        ...
        combined_image = draw_bounding_boxes(image.copy(),image_name, ground_truth, predictions, class_names)
        # Log images
...
def compute_errors_and_log(ground_truth_path, predicted_path):
    ...
    if num_files > 0:
        ...
        print(f"Mean Relative Difference (MRD): {mrd}")  # Print MRD

        # Log to wandb
        wandb.log({"prediction_RMSE": rmse, "prediction_MAE": mae, "prediction_MRD": mrd})
        wandb.run.summary["prediction_RMSE"] = rmse
        wandb.run.summary["prediction_MAE"] = mae
        wandb.run.summary["prediction_MRD"] = mrd  # Log MRD
    else:
...
class WandbCallback:
    ...
    def on_model_save(self, trainer) -> None:
      ...
      if trainer.best_fitness == trainer.fitness:
        ...
        self.run.log_artifact(
            ...
            type="model",
            name=f"{self.run.name}_{trainer.args.task}.pt",
            aliases=["last_best", f"epoch_{trainer.epoch + 1}"],
        )
    ...
    def on_val_end(self,validator):
        if validator.training==False:
          ...
          data_to_log = {
              "prediction/mp": results[0],
              "prediction/mr": results[1],
              "prediction/map50": results[2],
              "prediction/map": results[3]
          }
    ...
    def on_pred_end(self,predictor):
            images_folder = "/content/video-13-1/test/images"
            ground_truth_folder = "/content/video-13-1/test/labels"
