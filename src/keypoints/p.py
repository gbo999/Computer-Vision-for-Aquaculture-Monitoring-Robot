...
numOfKpts = 2 #replace this with the number of keypoints inside the dataset
kptsInfo  = numOfKpts * 3 

parser.add_argument(
    "--coco_json_path",
    default="dataset 2024-05-27 12-14-08/annotations/instances.json",
    type=str,
...
def main(root_dir, ana_txt_save_path_txt, json_file):
    ...
    images = {str(x["id"]): x for x in data["images"]}  # Ensure IDs are strings
    # Create image-annotations dict
    imgToAnns = defaultdict(list)
    for ann in data["annotations"]:
        imgToAnns[str(ann["image_id"])].append(ann)  # Ensure IDs are strings

    list_file = open(os.path.join(root_dir, "train.txt"), "w")

    # Write labels file
    for img_id, anns in tqdm(imgToAnns.items(), desc=f"Annotations {json_file}"):
        img_id_str = str(img_id)  # Ensure img_id is a string
        if img_id_str in images:
            img = images[img_id_str]

            h, w, f = img["height"], img["width"], img["file_name"]
            print(f"Processing {f}...")
        else:
            print(f"Image ID {img_id} not found in images dictionary.")
        ...
        dir_name = os.path.dirname((fn / f).with_suffix(".txt"))
